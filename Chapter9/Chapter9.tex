...
\begin{frame}{代码复用}
\begin{itemize}
  \item<1-> 函数---面向过程: $void ~fun()\{\}$
  \item<2-> 类---基于对象: $class ~mytype\{\};$
  \item<3-> 模板---泛型编程 : $template<typename ~T>$
  \item<4-> 继承---面向对象
\end{itemize}
...
\begin{figure}
  \centering
  \includegraphics[width=0.7\textwidth]{Fig/Constr-Destr.png}
\end{figure}

\only<2->{D3 obj;}

\uncover<2->{构造顺序：B} \uncover<3->{D1} \uncover<4->{D2} \uncover<5->{D3}\uncover<6->{(c1} \uncover<7->{c2} \uncover<8->{c3)}

\uncover<9->{析构顺序：D3}\uncover<10->{(c3} \uncover<11->{c2} \uncover<12->{c1)} \uncover<13->{D2} \uncover<14->{D1} \uncover<15->{B}
