...
\begin{block}<4->{this~指针:指向调用成员函数对象的\alert{const指针（指针常量）}，运行代码演示}
...
\vspace{-2.5mm}\begin{lstlisting}
Fraction::Fraction(const Fraction &rhs) : m_numerator(rhs.m_numerator), m_denominator(rhs.m_denominator) { cout<< "Copy constructor called" << endl; }
Fraction c2 = Fraction(3, 2);
\end{lstlisting}\vspace{-1.5mm}
\end{greenblock}
\begin{block}<2->{复制优化}
\alert{拷贝去除}技术：创建临时对象和调用拷贝构造函数这两个过程用直接初始化的方式来完成。
\end{block}
