...
\begin{block}<4->{this~指针:指向调用成员函数对象的\alert{const指针}，运行代码演示}
...
\begin{block}<1->{构造函数}
\begin{itemize}
  \item 调用：创建类类型对象会触发类的构造函数；
  \item 作用：为待创建的对象数据成员执行\alert{初始化}操作。
\end{itemize}
\end{block}
...
\begin{itemize}
  ...
  \item 功能：销毁对象的所有成员，释放内存；
\end{itemize}
\end{block}
\begin{blueblock}{语法：名字由\alert{波浪号}紧接类名构成，不能有返回值，也不能包含形参}
 \vspace{-2.5mm}\begin{lstlisting}
class Fraction {
public:
    ~Fraction() { } //析构函数
...
类成员函数还是类的辅助函数：
\begin{itemize}
  \item \alert{赋值（=）、下标（[]）、函数调用（()）和成员访问箭头（->）}运算符必须是类成员；
  \item \alert{改变运算对象自身状态}的运算符应该是类成员，比如复合赋值、自增、自减运算符等；
  \item \alert{具有对称性}的运算符一般应作为类的\alert{辅助函数}，比如算术、关系、逻辑运算符等。
\end{itemize}
\end{block}
\end{frame}
...
\begin{itemize}
  ...
  \item<2-> 含有类类型的混合类型表达式，运算符应该定义成辅助函数，通常定义为友元函数。
\begin{lstlisting}
class Fraction {
...
\begin{yellowblock}<3->{必须返回~ostream~类对象的引用，不允许复制IO类对象}
\begin{lstlisting}
Fraction a, b;
cout << a << b << endl;
\end{lstlisting}\vspace{-2mm}
...
\begin{frame}[fragile]
    {6.3.3 输入和输出运算符}
    \begin{itemize}
      \item 输入/输出运算符必须为类的辅助函数，不宜作为类的成员函数，例如
    \end{itemize}
...
\begin{blueblock}<1->{示例：利用关键字~ \textcolor{blue}{static}~为~Fraction~类添加静态成员}
\begin{lstlisting}
...
const int PartTimeWorker::ms_maxHourWeek;       //在类内部已有初始值，在类外部不可以再提供初始值
\end{lstlisting}\vspace{-1.5mm}
\end{blueblock}
\vspace{-1.5mm}
