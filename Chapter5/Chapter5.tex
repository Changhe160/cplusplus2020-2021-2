...
\centering
\fbox{\includegraphics[scale=0.23]{八皇后}}

\column{0.43\textwidth}
    这样的程序设计方法有没有问题？
    \begin{block}<2->{所有代码都在主函数中}
   \begin{itemize}
     \item  可读性差；
     \item  逻辑分析困难；
     \item 调试困难；
...
\begin{frame}[fragile]
    {5.2 认识函数\small{~---~存储周期}}
\begin{block}{自动存储周期}
定义在\alert{函数体或语句块内部的对象（包括函数的形参）}。在程序执行到其定义的位置时创建，离开其作用域时被释放，在\alert{栈}（stack\index{S!stack}）区分配存储空间。
\end{block}
...
为了支持并行程序设计，\alert{C++11}~引入了~\textcolor{blue}{thread\_local}~关键字。
\end{block}
\end{frame}
...
\column{0.45\textwidth}
\centering
\includegraphics[scale=0.5]{functioncall.png}
\end{columns}
\vspace{-1.5mm}
\end{blueblock}
\end{frame}
...
\end{itemize}
\end{block}
\begin{blueblock}<4->{请问程序输出结果？}\vspace{-1.5mm}
\begin{columns}
\column{0.55\textwidth}
\begin{lstlisting}
int fun() {
...
\begin{itemize}
  ...
  \item<3-> 全局对象具有外部链接性，可在文件间共享；
  \item<4-> ~\textcolor{blue}{static}~修饰的全局对象具有内部链接性 ，只能由同一个文件中的函数共享。
\end{itemize}
\end{block}
\begin{columns}
...
将实参的值拷贝给形参。
\begin{itemize}
  \item 安全：改变形参的值，不会影响到实参；
  \item 效率低：有时空代价。
\end{itemize}
\end{block}
\begin{blueblock}<2->{普通值传递的例子}\vspace{-1.5mm}
\begin{columns}
\column{0.65\textwidth}
\begin{lstlisting}
...
\column{0.3\textwidth}
\includegraphics[scale=0.08]{swap.png}
\end{columns}
\vspace{-1.5mm}
\end{blueblock}
...
\begin{frame}[fragile]
    ...
    \begin{block}{const~形参：兼顾效率与安全}
\begin{itemize}
  \item 避免内容的拷贝：高效性；
  \item 不能通过形参改变实参：安全性。
\end{itemize}
\end{block}
\begin{block}<2->{const~形参定义}
...
void print(char *str, unsigned size) {
    for (unsigned i = 0; i < size; ++i)        cout << str[i];
}
\end{lstlisting}
\vspace{-2mm}
\end{blueblock}
...
$\bullet$ 如果有精确匹配的函数，则调用此函数；否则选择实参与形参类型最接近的转换函数；\\
$\bullet$ 如果有一个以上无法区分的匹配，则会出现二义性调用错误；\\
$\bullet$ 如果找不到任何一个与实参相匹配的函数，则会出现无匹配错误。
\end{block}
\end{columns}
\end{frame}
...
\begin{itemize}
  ...
  \item 不足：函数调用中执行流的转移、参数传递、返回值处理等，需要时空开销。
\end{itemize}
\end{block}
\begin{blueblock}<2->{示例}
...
\end{blueblock}
\begin{block}<3->{减少开销}
编译器将在调用处嵌入函数代码，不会发生函数调用，因而也没有函数调用开销。
\end{block}
\begin{redblock}<4->{注意}
...
\vspace{-2.5mm}\begin{lstlisting}
int d = 5;
vector<int> ns{ 1, 2, 3, 4, 5, 10, 15, 20, 25, 35, 45, 50 };
for_each(ns.begin(), ns.end(), [d](int y) {     //d为外围d的拷贝
    if (y % d == 0)     cout << y << endl;
});
\end{lstlisting}\vspace{-1.5mm}
...
for_each(ns.begin(), ns.end(), [d, &sum](int y) { //sum为外围sum的引用
    if (y % d == 0)
        sum += y;
});
