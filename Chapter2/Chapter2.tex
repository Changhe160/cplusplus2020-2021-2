...
\begin{block}{对象}
    \begin{spacing}{1.2}
        \begin{itemize}
        \item 对象是\alert{数据和操作}的载体，当创建一个对象时，会为它分配一定内存空间
...
\begin{columns}
\column{0.6\textwidth}
一般格式：\alert{<数据类型名>~~~~对象名 ~{\large \textbf{;}}}\\
~\\
例如：
\begin{lstlisting}
...
\column{0.3\textwidth}
\includegraphics[scale=0.45]{object.png}
\end{columns}
...
%---------------------------------------------------------------------------------------------
\begin{frame}[fragile]{2.3 对象\normalsize{~---~作用域和生命期}}
\textcolor{blue}{作用域}：指标识符在代码中的\alert{可见范围}，通常以花括号分隔\\
\textcolor{blue}{生命期}：指对象在内存\alert{从产生到消亡}的时间。具有块域的对象，通常其生命期从定义处开始，在作用域结束时消亡
\pause
\vspace{-3mm}
\begin{columns}[t]
\column{0.5\textwidth}
...
\begin{columns}
			...
			\end{column}
\begin{column}{0.93\textwidth}\vspace{-3mm}
...
price car=1.0e5, mobile=100; //car和 mobile代表价格
...
\begin{block}{左值和右值}
	\begin{itemize}
        \item 左值所在的内存空间的地址是可以获取的（使用\underline{取址符 \texttt{\&}}），但右值的地址一般是无法得到的
...
const int N = 100;    //正确： N 为右值对象（使用时）
...
\begin{block}{优先级和结合性}
	\begin{itemize}
        ...
        \item \alert{结合性}：决定优先级相等的运算符结合在一起时的运算次序，同一优先级的运算符有相等的结合性（\alert{左/右结合}）。例如~+~和~-~的结合性是\alert{从左到右（左结合）}
...
\begin{yellowblock}<7->{说明}
 $\bullet$ 整型数的算术运算的结果还是整数，例如：\texttt{21/6}的结果是\texttt{3}，\alert{余数被舍弃}\\
 $\bullet$  求余运算中的两个运算数必须是\alert{整型类型}，如：\texttt{5\%3}\\
 $\bullet$ 如果运算对象的数据类型不相同，编译器会自动进行类型转换,原则是\alert{小数据类型向大数据类型转换}\\
 $\bullet$  注意数据溢出问题\\
  $\bullet$ \onslide<8->{计算结果存放在哪里？}
\end{yellowblock}
...
\end{lstlisting} \vspace{-2mm}
\pause
    \item 赋值运算符的优先级较低，有时需要加上括号才能正确执行，例如：
\pause \vspace{-2mm}
\begin{lstlisting}
...
第二行代码首先计算\texttt{couter+5}的值，然后将结果放到一个临时对象里，再取值赋值给\texttt{counter}。为简化计算，\texttt{C++}提供了\alert{复合运算符(\texttt{+=}~~~\texttt{-=}~~~\texttt{/=}~~~\texttt{*=}~~~\texttt{\%=})}
\end{yellowblock}
...
\hline
\texttt{\&\&}&\alert{逻辑与}&左&	5\texttt{\&\&}4\\
\hline
\texttt{||}&\alert{逻辑或}&左&	5\texttt{||}4\\
\hline
\end{tabular}}}
\end{center}
\end{table}
...
\begin{frame}[fragile]{2.5 表达式\normalsize{~---~逻辑和关系运算符}}
\begin{center}
可以用\texttt{~i~$<=$~j~$<=$~k}直接构造\alert{数学表达式}\texttt{$i\leq j\leq k$}的逻辑关系吗？
\end{center}
\pause
\begin{columns}[t]
\column{0.9\textwidth}
